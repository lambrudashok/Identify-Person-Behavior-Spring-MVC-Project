...
import com.model.PredictionModel;
import com.model.TextDocumentDataModel;
...
public class PredictionRepository {
	...
	public List<PostLayoutModel> viewAllUserPosts(int registerid){
		...
		try {
			// store posts id users in List
			List<Integer> alPost = template.query("select pm.postid from postmaster pm "
					...
					+ "order by pm.postdate desc", new Object[] {registerid}, new RowMapper<Integer>() {
						@Override
			...
			System.out.println("prediction repository error :"+e);
			return null;
		}
	}
		...
		public String ViewPost(int postid){
			try {
				String post = template.queryForObject("select post from postmaster where postid=?", new Object[] {postid}, new RowMapper<String>() {
					@Override
		...
		// k means clustering
			public int kMeansClustering(int[] postVectors,String[] afterStopWord) {
				...
				if(predict==cluster.get(2)) {
					...
					System.out.println("error prediction:"+e);
					return 0;
				}
			}
		...
		public String[] getAllPostsCommentsLikes(int predictUserID) {
			final List<TextDocumentDataModel> list = new LinkedList<TextDocumentDataModel>(); // store particular user posts
			try {
				// fetch all posts
				template.query("select pm.post from postmaster pm "
						...
						+ "where rm.registerid=?", new Object[] {predictUserID}, new RowMapper<Void>() {
							@Override
							public Void mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				template.query("select comment from commentmaster where registerid=?", new Object[] {predictUserID}, new RowMapper<Void>() {
					@Override
					public Void mapRow(ResultSet rs, int rowNum) throws SQLException {
						TextDocumentDataModel info = new TextDocumentDataModel();
				...
				template.query("select pm.post from likemaster lm "
				                + "inner join likepostjoin lpj on lpj.likeid=lm.likeid "
								+ "inner join postmaster pm on pm.postid=lpj.postid "
				                + "where lm.registerid=?", new Object[] {predictUserID}, new RowMapper<Void>() {
									@Override
									public Void mapRow(ResultSet rs, int rowNum) throws SQLException {
										TextDocumentDataModel info = new TextDocumentDataModel();
				...
				System.out.println("error repo prediction :" + e);
				return null;
			}
		}
